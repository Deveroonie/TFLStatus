...
function DLRStatus() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get("https://api.tfl.gov.uk/line/mode/dlr/status");
    ...
    <div className="grid grid-cols-1 gap-4 text-white" style={{paddingTop: "16px"}}>
      {response.map(data => (
        <div key={data.id} className="p-4 bg-gray-800 rounded-lg">
          ...
          >
            {data.name} - {data.lineStatuses[0].statusSeverityDescription}&nbsp;<img src="/dlr.png" width={64} h={64} alt={"DLR logo"}></img>
          </div>
